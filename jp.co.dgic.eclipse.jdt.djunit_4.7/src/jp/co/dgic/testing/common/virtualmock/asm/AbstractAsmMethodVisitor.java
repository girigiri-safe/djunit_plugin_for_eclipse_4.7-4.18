...
public abstract class AbstractAsmMethodVisitor extends MethodVisitor implements Opcodes {
	...
	public void createMethodCall(int opcode, String owner, String name, String desc, boolean itf) {
		...
		mv.visitMethodInsn(INVOKESTATIC, MANAGER_CLASS_NAME, "indicateCalledAndGetReturnValue", "(Ljava/lang/String;[Ljava/lang/Object;)Ljava/lang/Object;", false);
	...
	protected void createReturnValueProcess() {
		...
		InternalMockObjectManager.printConsole("### AbstractAsmMethodVisitor.createReturnValueProcess: " + this._className + "." + this._methodName);
		InternalMockObjectManager.printConsole("#################################################################");
...
//    		mv.visitLabel(toReturn);
	...
	protected void createInvoleThrowException(String className, String methodName) {
		...
		mv.visitMethodInsn(INVOKESTATIC, MANAGER_CLASS_NAME, "throwException", "(Ljava/lang/Object;Ljava/lang/String;)V", false);
	}
	// for begin method
	protected void createCreateArgsArray(boolean isStaticMethod, Type[] argTypes, int varStartIndex) {
		...
		for (int i = 0; i < argTypes.length; i++) {
			...
			if (isPrimitive(argTypes[i])) {
				...
				mv.visitMethodInsn(INVOKESPECIAL, wrapperType, CONSTRUCTOR_METHOD_NAME, "(" + argTypes[i] + ")V", false);
...
//			t.printStackTrace();
	...
	protected boolean canReplace(String className, boolean isInterface) {
		...
		if (DJUnitUtil.isExcluded(name)) { return false; }
