...
//	}
	...
	public String[][] getClasspathAndModulepath(ILaunchConfiguration configuration) throws CoreException {
		String[][] cp = super.getClasspathAndModulepath(configuration);
		return addDJUnitClassPath(configuration, cp);
	}
...
//	}
	...
	private String[][] addDJUnitClassPath(ILaunchConfiguration configuration, String[][] cp)
			throws CoreException {
		URL url = Platform.getBundle(DJUnitPlugin.PLUGIN_ID).getEntry("/");
		String[] classPath = cp[0];
		try {
			...
			String[] newClassPath = new String[classPath.length + junitEntries.size()];
	...
	private void addRequiredJars(ILaunchConfiguration configuration, List junitEntries, URL djunitURL)
		...
		if ((javaProject == null) || !javaProject.exists()) {
			abort(
					JUnitMessages.JUnitLaunchConfigurationTab_error_notJavaProject,
					null,
